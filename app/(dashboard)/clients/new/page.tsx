      ...
      <section className="mx-auto max-w-3xl px-4 py-6 md:px-6 md:py-8">
        ...
        >
          ...
          <input
            ...
            placeholder="Industry (e.g. HVAC, Interior Design)"
            className="w-full rounded-md bg-slate-900 p-3 text-sm outline-none focus:ring-1 focus:ring-cyan-400"
          />
          <input
            value={location}
            onChange={(e) => setLocation(e.target.value)}
            placeholder="Service location (e.g. Phoenix, AZ)"
            className="w-full rounded-md bg-slate-900 p-3 text-sm outline-none focus:ring-1 focus:ring-cyan-400"
          />
          <div>
            <input
          ...
          <input
            ...
            placeholder="Calendar booking link (Calendly, Cal.com, etc.)"
            className="w-full rounded-md bg-slate-900 p-3 text-sm outline-none focus:ring-1 focus:ring-cyan-400"
          />

          {/* Custom knowledge — the key GAP 3 addition */}
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-200">
              Custom knowledge
            </label>
            <p className="mb-2 text-xs text-slate-400">
              Anything the AI should know that&apos;s not on the website.
              Service area zip codes, pricing, brands serviced, diagnostic fees,
              special offers, team names — anything specific to this business.
              Plain text is fine.
            </p>
            <textarea
              value={customKnowledge}
              onChange={(e) => setCustomKnowledge(e.target.value)}
              placeholder={`Example:\n- Service area: 85001–85099 zip codes only\n- Diagnostic fee: $89 (waived if repair booked same day)\n- Brands serviced: Carrier, Trane, Lennox, Rheem\n- Emergency line: 602-555-0123\n- Not available Sundays`}
              rows={6}
              className="w-full rounded-md bg-slate-900 p-3 text-sm outline-none focus:ring-1 focus:ring-cyan-400"
            />
          </div>
        ...
        {crawlStatus && (
          <div className="mt-4 rounded-lg border border-white/10 bg-white/5 p-4">
            <p className="text-sm font-semibold text-slate-200">Website crawl results</p>
            <div className="mt-3 flex gap-6">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-emerald-400">{crawlStatus.pagesCrawled}</p>
              </div>
              {crawlStatus.errors.length > 0 && (
                <div>
                  <p className="text-xs text-slate-400">Errors</p>
                  <p className="text-lg font-semibold text-red-400">{crawlStatus.errors.length}</p>
                </div>
              )}
            </div>
            {crawlStatus.errors.length > 0 && (
              <div className="mt-3">
                <p className="text-xs font-medium text-red-400 mb-1">Pages that failed:</p>
                <ul className="space-y-1">
                  {crawlStatus.errors.map((err, i) => (
                    <li key={i} className="text-xs text-slate-400 break-all">• {err}</li>
                  ))}
                </ul>
              </div>
            )}
            {crawlStatus.pagesCrawled === 0 && crawlStatus.started && (
              <p className="mt-3 text-xs text-amber-400">
                ⚠ No pages crawled. The AI will have no website knowledge. Check the URL.
              </p>
            )}
            {crawlStatus.pagesCrawled > 0 && crawlStatus.errors.length === 0 && (
              <p className="mt-3 text-xs text-emerald-400">✓ All pages crawled. Redirecting...</p>
            )}
            <button
              onClick={() => router.push("/")}
